...
Each \(trap_i\) requires s steps and has c complexity. After each step of disarming a trap, Jimothy incurs fatigue based on the complexity of the trap with the most steps he has solved (none if it is the first).\\\\
...
Help Jimothy disarm all the traps with minimum fatigue so that he can keep exploring this never-ending trove.
\section*{Input}
...
The first line of input contains an integer n (constraint to be decided) indicating the number of traps followed by n lines for each trap containing 2 integers, \(steps_i\) and \(complexity_i\).\\
