...
The first line of input contains an integer 1 \(<=\) $N$ \(<= 10^5\) indicating the number of traps. 
The next $N$ lines each represent a \(trap_i\), where $i$ ranges from 1 to $N$. 
Each of these lines contains two integers: \(S_i\), which denotes the number of steps required to disarm \(trap_i\), 
and \(C_i\), which denotes its complexity.\\ \\
